	...
	while (option != 0) {
		switch (option) {
			case REGISTER_PRODUCT:
				...
				if (is_blank(name)) {
					showError("Nome do produto nÆo pode ser vazio");
					timeout(TIMEOUT);
					break;
				}
				...
				if (is_blank(unit)) {
					showError("Unidade do produto nÆo pode ser vazia");
					timeout(TIMEOUT);
					break;
				}
				...
				if (is_blank(address)) {
					showError("Endere‡o do produto nÆo pode ser vazio");
					timeout(TIMEOUT);
					break;
				}
			...
			case SEARCH_PRODUCT:
				...
				get_input(name, sizeof(name));
				if (is_blank(name)) {
					showError("Vocˆ deve informar algo para buscar");
					timeout(TIMEOUT);
					break;
				}
