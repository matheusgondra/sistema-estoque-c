...
bool stg_find_product(Product *product, int id) {
  ...
  for (size_t i = 0; i < list.size; i++) {
    if (list.items[i].id == id) {
      create_product_with_id(product, list.items[i].id, list.items[i].name,
                             list.items[i].unit, list.items[i].address);
...
bool stg_find_product_by_name(Product *product, const char *name) {
  ...
  for (size_t i = 0; i < list.size; i++) {
    if (strcmp(list.items[i].name, name) == 0) {
      ...
      create_product(product, list.items[i].name, list.items[i].unit,
                     list.items[i].address);
      product->quantity = list.items[i].quantity;
